...
if [ -n "$USEMSVC" ]; then
  ...
  # Download and compile an implementation of getopt that works with MSVC
  f=full-getopt-port-for-unicode-and-multibyte-microso
  rm -rf $f
  git clone -q https://git.codeproject.com/ludvik-jerabek/$f
  $CC -Wl,-dll -DEXPORTS_GETOPT -o usr/bin/libgetopt.dll $f/getopt_mb_uni_src/getopt.c
  cp $f/getopt_mb_uni_src/getopt.h usr/include

  # Fix an LLVM header bug for 64-bit MSVC
  sed -i 's/typedef signed int ssize_t;/typedef intptr_t ssize_t;/' usr/include/llvm/Support/DataTypes.h
